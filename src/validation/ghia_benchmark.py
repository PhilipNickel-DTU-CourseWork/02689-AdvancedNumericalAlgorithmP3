...
class GhiaBenchmark:
    """
    Benchmark data from Ghia et al. (1982) for lid-driven cavity validation.

    Contains velocity profile data along the cavity centerlines for various
    ...
    # Mapping of Reynolds numbers to data
    _DATA_MAP = {
        100: GHIA_RE_100,
        400: GHIA_RE_400,
        1000: GHIA_RE_1000,
    ...
    def get_data(cls, Re):
        """
        Get Ghia et al. benchmark data for the specified Reynolds number.

        Parameters
        ----------
        Re : int
            Reynolds number (100, 400, 1000, 3200, 5000, 7500, or 10000)

        Returns
        -------
        dict or None
            Dictionary containing 'x', 'y', 'u', 'v' arrays, or None if not available
            - 'x': x-coordinates along horizontal centerline (for v-velocity)
            - 'y': y-coordinates along vertical centerline (for u-velocity)
            - 'u': u-velocity values along vertical centerline at x=0.5
            - 'v': v-velocity values along horizontal centerline at y=0.5
        """
        return cls._DATA_MAP.get(Re)

    @classmethod
    def available_reynolds_numbers(cls):
        """
        Get list of available Reynolds numbers.
        ...
        """
        return list(cls._DATA_MAP.keys())

    @classmethod
    def get_closest_reynolds(cls, Re):
        """
        Find the closest available Reynolds number to the requested value.
        ...
        -------
        int
            Closest available Reynolds number
        """
        available = cls.available_reynolds_numbers()
        return min(available, key=lambda x: abs(x - Re))
